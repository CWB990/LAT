...
def seed_everything(seed=26):
    ...
    if torch.cuda.is_available():
        ...
        # 6. CuDNN 确定性算法 (关键！因为你用了 ConvNCF)
        # 这会让卷积变慢一点点，但能保证结果一致
        torch.backends.cudnn.deterministic = True
        torch.backends.cudnn.benchmark = False

def parse_args():
    parser = argparse.ArgumentParser(description="Run ConvNCF_TWP.")
    ...
    parser.add_argument("--fake_users_file", nargs="?", default="Data/lastfm_attack/lastfm/lastfm_random_head_0.02.json",
                        help="Input data path.")
    parser.add_argument("--attack_type", nargs="?", default="random",
                        choices=['entars', 'wmf'])
    parser.add_argument('--epochs', default=200, type=int)
    parser.add_argument('--fname', default='results/ConvNCF/TWP', type=str)
    # 25 控制有目标噪声大小的参数##################
    ...
    for epoch in range(epoches):
       ...
       # === 逻辑判断 ===
        ...
        if epoch >= cnn_start_epoch:
            ...
            awp_adversary = utils_convNCF.AdvWeightPerturb_ConvNCF_TWP(model=model, proxy=proxy_adv, proxy_optim=proxy_opt, gamma=temp_awp_gamma)
        ...
        # if epoch >=2:
            ...
            if hr10 >= best_hr10:
                ...
                results = utils_convNCF.calculate_target_metrics_TWP(model, dataset.target_items, dataset.target_users,
                ...
                best_model_path = os.path.join(path, 'pretrained', f"{args.dataset}_{args.attack_type}_ConvNCF_TWP_{time.time()}.pth")
...
def layer_sharpness(args, model_adv, train_group, train_loader):
    ...
    target_users, unrated_items_dict = utils_convNCF.get_target_users_and_unrated_items(
...
if __name__ == '__main__':
    ...
    model = ConvNCF(args.nUsers, args.nItems, device)
    proxy_adv = ConvNCF(args.nUsers, args.nItems, device)
